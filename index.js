  ...
  ws.on("message", function incoming(message) {
    ...
    switch (msg.event) {
      ...
      case "media":
        ...
        // Build the wav file from scratch for raw data
        let wav = new WaveFile();

        // Twilio uses MuLaw for encoding
        wav.fromScratch(1, 8000, "8m", Buffer.from(twilioData, "base64"));
        
        // Library method to decode MuLaw to 16-bit PCM
        wav.fromMuLaw();
        ...
        // We have to chunk data b/c twilio sends audio durations of ~20ms, AssemblyAI needs a min of 100ms
        if (chunks.length >= 5) {
...
app.post("/", async (req, res) => {
  assembly = new WebSocket(
    ...
    { headers: { authorization: "f54d5fd748aa42a1a1e3aaf70e67d0cc" } }
  );
